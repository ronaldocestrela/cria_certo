...
public class ToggleFeatureFlagCommandHandler : IRequestHandler<ToggleFeatureFlagCommand, Result<FeatureFlagDto>>
{
    private readonly DbContext _dbContext;

    public ToggleFeatureFlagCommandHandler(DbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public async Task<Result<FeatureFlagDto>> Handle(ToggleFeatureFlagCommand command, CancellationToken cancellationToken)
    {
        ...
        var oldStateJson = JsonSerializer.Serialize(new { flag.IsEnabled });
        var toggleResult = flag.Toggle(command.IsEnabled, command.ActorEmail, command.Reason.Trim());
        if (toggleResult.IsFailure)
        {
            return Result.Failure<FeatureFlagDto>(toggleResult.Error);
        }

        var newStateJson = JsonSerializer.Serialize(new { flag.IsEnabled });

        // Auditoria forense SHA-256 encadeada
